...
#----
#원리
#소를 관찰할때마다 관찰 결과를 observe라는 딕셔너리에 저장해주고
#만약 새로 관찰한 소의 위치가 딕셔너리에 저장된 위치와 다르면, 
#cnt를 1 증가해주고, 딕셔너리의 위치를 수정해준다.

import sys
sys.stdin = open('3_소가길을건너간이유1.txt', 'r')
...
observe = dict()
for i in range(T):
    id, location = map(int, input().split())

    if id not in observe:
        observe[id] = location
    else:
        if observe[id] != location:
            observe[id] = location
            cnt += 1
print(cnt)