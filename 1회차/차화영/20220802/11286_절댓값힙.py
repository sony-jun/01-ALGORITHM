...
N = int(input())
arr = [] # 절대값이 가장 작은 값을 출력하기 위한 heap
for i in range(N):
    x= int(input()) # 정수 x 입력 받음
    
    # if x > 0:                           # x가 양수일 때
    #     heapq.heappush(arr, (x, x))     # arr에 (절대값, 실제값) 추가
    
    # elif x < 0:                         # x가 음수일 때
    #     heapq.heappush(arr, (-x, x))    # arr에 (절대값, 실제값) 추가 ; x가 음수이면 -를 곱해야 양수로 절대값이 됨!
    ...
    if x != 0:
        ...
        if len(arr) == 0: # 배열의 길이가 0 -- 비어 있을 때 (if not num: 라고 써도 됨)
            ...
            print(heapq.heappop(arr)[1]) # [1]번째 인덱스의 값 -- x를 출력