...
while len(numbers) > 1:
    ...
    garbage.append(numbers.pop(0)) # numbers에서 맨 앞 값을 버린 후 그 숫자를 garbage에 넣는다.
    numbers.append(numbers.pop(0)) # numbers에서 맨 앞 값을 버린 후 그 숫자를 numbers의 맨 뒤에 넣는다.
...
#len(numbers) 길이가 1로 while의 조건에 false이므로.. 반복 종료
